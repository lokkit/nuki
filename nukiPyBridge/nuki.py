...
class Nuki():
	...
	#	-my_public_key_hex: a public key (as hex string) you created to talk with the Nuki Lock
	...
	def authenticateUser(self, my_public_key_hex, privateKeyHex, ID, IDType, name):
		...
		public_key_from_nuki = commandParsed.publicKey
		self.config.set(self.macAddress,'public_key_from_nuki',public_key_from_nuki)
		self.config.set(self.macAddress,'my_public_key_hex',my_public_key_hex)
		...
		self.config.set(self.macAddress,'Name',name)
                print "Response: Public key received: %s" % public_key_from_nuki

                print "==================="
		publicKeyPush = nuki_messages.Nuki_PUBLIC_KEY(my_public_key_hex)
		publicKeyPushCommand = publicKeyPush.generate()
		print "Pushing my public key (%s) using command: %s"%(my_public_key_hex, publicKeyPush.show())
		self._charWriteResponse = ""
		self.device.char_write_handle(pairingHandle,publicKeyPushCommand,True,5)
		print "Public key pushed"
		commandParsed = self.parser.parse(self._charWriteResponse)
                if self.parser.isNukiCommand(self._charWriteResponse) == False:
                        sys.exit("Error while pushing public key: %s (_charWriteResponse '%s')" % (commandParsed, self._charWriteResponse))
                if commandParsed.command != '0004':
                        sys.exit("Nuki returned unexpected response (expecting CHALLENGE): %s" % commandParsed.show())
                print "Response: Challenge received: %s" % commandParsed.nonce

                print "==================="
                nonceNuki = commandParsed.nonce
		authAuthenticator = nuki_messages.Nuki_AUTH_AUTHENTICATOR()
		authAuthenticator.createPayload(nonceNuki, privateKeyHex, my_public_key_hex, public_key_from_nuki)
		...
		authData.createPayload(public_key_from_nuki, privateKeyHex, my_public_key_hex, nonceNuki, ID, IDType, name)
		...
		authIDConfirm.createPayload(public_key_from_nuki, privateKeyHex, my_public_key_hex, nonceNuki, authId)
		...
		with open('./nuki.cfg', 'wb') as configfile:
	...
	def readLockState(self):
		...
		stateReqEncrypted = nuki_messages.Nuki_EncryptedCommand(authID=self.config.get(self.macAddress, 'authorizationID'), nukiCommand=stateReq, publicKey=self.config.get(self.macAddress, 'public_key_from_nuki'), privateKey=self.config.get(self.macAddress, 'privateKeyHex'))
		...
		commandParsed = self.parser.decrypt(self._charWriteResponse,self.config.get(self.macAddress, 'public_key_from_nuki'),self.config.get(self.macAddress, 'privateKeyHex'))[8:]
	...
	def lockAction(self,lockAction):
		...
		challengeReqEncrypted = nuki_messages.Nuki_EncryptedCommand(authID=self.config.get(self.macAddress, 'authorizationID'), nukiCommand=challengeReq, publicKey=self.config.get(self.macAddress, 'public_key_from_nuki'), privateKey=self.config.get(self.macAddress, 'privateKeyHex'))
		...
		lockActionReqEncrypted = nuki_messages.Nuki_EncryptedCommand(authID=self.config.get(self.macAddress, 'authorizationID'), nukiCommand=lockActionReq, publicKey=self.config.get(self.macAddress, 'public_key_from_nuki'), privateKey=self.config.get(self.macAddress, 'privateKeyHex'))
	...
	def getLogEntriesCount(self, pinHex):
		...
		logEntriesReqEncrypted = nuki_messages.Nuki_EncryptedCommand(authID=self.config.get(self.macAddress, 'authorizationID'), nukiCommand=logEntriesReq, publicKey=self.config.get(self.macAddress, 'public_key_from_nuki'), privateKey=self.config.get(self.macAddress, 'privateKeyHex'))
	...
	def getLogEntries(self,count,pinHex):
		...
		for message in messages:
			...
			try:
				commandParsed = self.parser.decrypt(message,self.config.get(self.macAddress, 'public_key_from_nuki'),self.config.get(self.macAddress, 'privateKeyHex'))[8:]
				if self.parser.isNukiCommand(commandParsed) == False:
					sys.exit("Error while requesting Nuki Log Entries: %s" % commandParsed)
				commandParsed = self.parser.parse(commandParsed)
